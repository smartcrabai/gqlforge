...
pub async fn introspect(connection_url: &str) -> Result<DatabaseSchema> {
    ...
    // Fetch tables, views, and their metadata (columns, primary keys, foreign keys, unique constraints) in parallel per table.
    let tables_query = r"
    ...
    // Materialized views use pg_catalog.pg_matviews since information_schema omits them (relkind = 'm').
